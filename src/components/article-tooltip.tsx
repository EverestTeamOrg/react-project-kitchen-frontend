...
import open from "../images/open-article-icon.svg";
...
import {
  TooltipAuthorWrapper,
  TooltipButton,
  TooltipContainer,
  TooltipIcon
} from "./StyledComponents/article-tooltip-styles";
...
export const ArticleTooltip: FunctionComponent<{ article: TArticleProperties }> = (props) => {
  ...
  return (
    <TooltipContainer>
      <TooltipAuthorWrapper>
        <ProfileInformationView articleDate={composeCreatedDate(props.article.createdAt)}
                                author={props.article.author}/>
        <TooltipButton onClick={() => {
          push(`/article/${props.article.slug}`);
        }}>
          <TooltipIcon src={open} alt="Читать далее" />
        </TooltipButton>
      </TooltipAuthorWrapper>
